...
**第二阶段：仿射密码置换。** 将线性序号 `i` 映射为 `y = (A*i + B) mod N`，其中 `N = 62^6 ≈ 568 亿`。参数 A 必须与 62 互素（启动时校验），预计算模逆元 `INV_A`。这是一个双射（一一对应），保证序号唯一则短码唯一，同时让连续序号变得不可预测。

**第三阶段：定长 Base62 编码。** 将置换后的值编码为恰好 6 位 `[0-9A-Za-z]` 字符。

反向解码同样支持：Base62 解码 → 逆仿射 → 还原序号，用于 `mightExist()` 快速否定判断。
...
| L3 | ShortCodeUtil.mightExist() | 解码短码反算序号，与游标比较快速否定 | 实时 |
| L4 | Bloom Filter | 创建时写入，快速否定不存在的短码 | 持久化 |
| L5 | 空值缓存 | 缓存"不存在"的结果，防穿透 | 30min |
| L6 | DB 查询 | 最终兜底：t_link_goto → t_link | — |
